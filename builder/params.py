...
class Params:
    ...
    def with_web_id(self, auth=None, url="", fake=False):
        self.params["webid"] = (
            generate_fake_webid() if fake else generate_webid(auth, url)
        )
        return self

    def with_a_bogus(self, data=None):
        query = splice_url(self.get())
        encoded_data = splice_url(data) if data is not None else ""
        self.add_param("a_bogus", generate_a_bogus(query, encoded_data))
        return self

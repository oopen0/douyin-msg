...
from utils.dy_util import generate_millisecond, generate_req_sign, generate_webid
...
class ProtoBuilder:
    ...
    def build_normal_request(auth, cmd):
        ...
        request.sdk_cert = base64.b64encode(
            auth.client_cert.encode("utf-8")
        ).decode("utf-8")
        return request

    @staticmethod
    def build_create_conversation_request(auth, to_id, my_id):
        request = ProtoBuilder.build_normal_request(auth, 609)
        request.body.create_conversation_v2_body.conversation_type = 1
        request.body.create_conversation_v2_body.participants.extend(
            [int(to_id), int(my_id)]
        )
        request.reuqest_sign = generate_req_sign(
            {
                "sign_data": (
                    f"avatar_url=&idempotent_id=&name=&participants={to_id},{my_id}"
                ),
                "certType": "cookie",
                "scene": "web_protect",
            },
    ...
    def build_send_message_request(
        auth, conversation_id, conversation_short_id, ticket, message
    ):
        ...
        compact_content = json.dumps(
            msg_content, ensure_ascii=False, separators=(",", ":")
        )
        ...
        request.body.send_message_body.content = compact_content
